...
bank_choice = {"1": "开户", "2": "存钱", "3": "取钱", "4": "转账", "5": "查询", "6": "再见"}  # 银行业务选项
# 开户成功的信息模板
myinfo = '''
\033[0;32;40m
    ------------账户信息------------
...
def getRandom():
    ...
    for i in range(8):
        ...
        print(li[int(random.random() * len(li))])
    return string
...
# 银行的开户方法
def bank_addUser(username, password, country, province, street, door, money,moneytype):
    # 查询是否已满
    if len(bank) >= 100:
        return 3
    # 查询是否存在
    ...
    # 插入数据
        ...
        bank[username] = {"account":getRandom(),"password": password, "country": country, "province": province, "street": street,
                          "door": door, "money": money, "moneytype":moneytype,"bank_name": bank_name}
        return 1
    ...
    if uname != None:
        if bank[uname]["password"] == password:
            if bank[uname]["money"] < money:
                ...
                if bank[uname]["moneytype"] == '金卡':
                    if money <=50000:
                        bank[uname]["money"] -= money
                        print("办理成功")
                    elif money > 50000:
                        print("金卡最多出账五万，办理不成功")
                elif bank[uname]["moneytype"] == '普通卡':
                    if money <=20000:
                        bank[uname]["money"] -= money
                        print("办理成功")
                    elif money > 20000:
                        print("普通最多出账两万，办理不成功")
                return 0
        else:
            return 2
    else:
...
def addUser():
    ...
    country = inputHelp("居住地址：国家")
    province = inputHelp("省份")
    street = inputHelp("街道")
    door = inputHelp("门牌号")
    money = inputHelp("银行卡余额", "int")
    if money >= 50000:
        a=input("请选择是否办理金卡(Y OR N)：")
        if a == 'Y' or 'y':
            moneytype = '金卡'
        elif a == 'N' or 'n':
            moneytype = '普通卡'
    ...
    status = bank_addUser(username, password, country, province, street, door, money,moneytype)
    # 判断1   2   3
    if status == 1:
        user = bank[username]
